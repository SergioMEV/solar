 ...
 * 
 * \param server_name  A null-terminated string that specifies either the IP address
 *                     or host name of the server to connect to.
 * \param port         The port number the server should be listening on. 
 *
 * \returns  A file descriptor for the connected socket, or -1 if there is an error. 
 *           The errno value will be set by the failed POSIX call. 
 */
static int socket_connect(char* server_name, unsigned short port) {
    // Look up the server by name
    struct hostent* server = gethostbyname(server_name);
    if (server == NULL) {
        // Set errno, since gethosthame does not
        errno = EHOSTDOWN;
        return -1;
    }
 ...
 *
 * \returns     A file descriptor for the serer socket. The socket has been 
 *              bound to a particular port and address, but is not listening.
 *              In case of failure, this function returns -1. The value of 
 *              errno will be set by the POSIX socket fun tion that failed. 
 */
 ...
 */
static int serevr_socket_accept(int server_socket_fd) {
